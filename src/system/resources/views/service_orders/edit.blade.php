...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{ route('service-orders.show', $order) }}">OS #{{ $order->id }}</a></li>
...
<div class="row">
  ...
  <div class="col-md-8 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h6 class="card-title">Editar Ordem de Serviço #{{ $order->id }}</h6>

        <form class="forms-sample" method="POST" action="{{ route('service-orders.update', $order) }}" novalidate>
          @csrf
          @method('PUT')
          ...
          <div class="mb-3">
            ...
            <select class="form-select @error('customer_id') is-invalid @enderror"
              ...
              @foreach($clients as $id => $name)
                <option value="{{ $id }}"
                  {{ (string)$id === (string)old('customer_id', $order->customer_id) ? 'selected' : '' }}>
                  {{ $name }}
                </option>
              @endforeach
            </select>
            ...
            <small class="text-muted d-block mt-1">
              Alterar o cliente atualiza os dados de snapshot (nome, telefone, e-mail) desta OS.
            </small>
          </div>
            ...
            <div class="col-md-6">
              ...
              <div class="mb-3">
                ...
                <input type="datetime-local"
                       ...
                       value="{{ old('opened_at', optional($order->opened_at)->format('Y-m-d\TH:i')) }}">
          ...
          @php
  // mapa dos novos valores (value => label)
  $serviceTypes = [
  ...
  // normaliza valores antigos -> novos
  $raw = old('service_type', $order->service_type ?? 'maintenance');
  $serviceTypeValue = match ($raw) {
      'install'     => 'installation',
      'corrective'  => 'maintenance',
      'preventive'  => 'maintenance',  // ajuste se preferir outro mapeamento
      'other'       => 'maintenance',  // idem
      default       => $raw ?: 'maintenance',
  };
@endphp
...
@error('service_type') <div class="invalid-feedback">{{ $message }}</div> @enderror
  ...
  $problemCatValue = old('problem_category', $order->problem_category ?? 'outros');
@endphp
...
@error('problem_category') <div class="invalid-feedback">{{ $message }}</div> @enderror
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control @error('problem_description') is-invalid @enderror"
                      ...
                      placeholder="Relate o problema informado pelo cliente...">{{ old('problem_description', $order->problem_description) }}</textarea>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control @error('services_done') is-invalid @enderror"
                      ...
                      placeholder="Descreva os procedimentos realizados...">{{ old('services_done', $order->services_done) }}</textarea>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control @error('parts_list') is-invalid @enderror"
                      ...
                      placeholder="Liste peças e materiais utilizados (quantidade, referência)...">{{ old('parts_list', $order->parts_list) }}</textarea>
            @error('parts_list') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>

          {{-- Operacional: horas, km, técnico --}}
          <div class="row">
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('labor_hours', number_format((float)$order->labor_hours, 2, '.', '')) }}">
            ...
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('travel_km', number_format((float)$order->travel_km, 2, '.', '')) }}">
            ...
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="text" class="form-control @error('technician_name') is-invalid @enderror"
                       ...
                       value="{{ old('technician_name', $order->technician_name) }}" placeholder="Nome do técnico">
            ...
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('technician_hour_cost_snapshot', number_format((float)$order->technician_hour_cost_snapshot, 2, '.', '')) }}">
            ...
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('travel_cost_per_km_snapshot', number_format((float)$order->travel_cost_per_km_snapshot, 2, '.', '')) }}">
            ...
            <div class="col-md-4">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('parts_cost', number_format((float)$order->parts_cost, 2, '.', '')) }}">
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="number" step="0.01" min="0"
                       ...
                       value="{{ old('total_amount', number_format((float)$order->total_amount, 2, '.', '')) }}" required>
            ...
            <div class="col-md-6">
              <div class="mb-3 form-check mt-4">
                <input type="checkbox"
                       ...
                       value="1" {{ old('resolved_in_first_visit', $order->resolved_in_first_visit) ? 'checked' : '' }}>
          ...
          {{-- Fechamento (opcional) --}}
          <div class="mb-3">
            <label class="form-label" for="closed_at">Data de fechamento</label>
            <input type="datetime-local"
                   class="form-control @error('closed_at') is-invalid @enderror"
                   id="closed_at" name="closed_at"
                   value="{{ old('closed_at', optional($order->closed_at)->format('Y-m-d\TH:i')) }}">
            @error('closed_at') <div class="invalid-feedback">{{ $message }}</div> @enderror
            <small class="text-muted">Defina apenas quando a OS for concluída.</small>
          </div>

          <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary">Salvar alterações</button>
            <a href="{{ route('service-orders.show', $order) }}" class="btn btn-secondary">Cancelar</a>
          </div>
        </form>
